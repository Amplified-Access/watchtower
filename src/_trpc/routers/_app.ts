...
export const appRouter = router({
  ...
  getDashboardStats: superAdminProcedure.query(async () => {
    try {
      ...
      const [watchersCount] = await db
        .select({ count: count() })
        .from(user)
        .where(eq(user.role, "watcher"));

      // Get growth data for organizations (this month vs last month)
      const [organizationsThisMonth] = await db
        .select({ count: count() })
        .from(organizations)
        .where(sql`${organizations.createdAt} >= ${currentMonth}`);

      const [organizationsLastMonth] = await db
        .select({ count: count() })
        .from(organizations)
        .where(
          and(
      ...
      const [adminsThisMonth] = await db
        .select({ count: count() })
        .from(user)
        .where(
          and(
      ...
      const [watchersThisMonth] = await db
        .select({ count: count() })
        .from(user)
        .where(
          and(
      ...
      const [criticalIncidentsCount] = await db
        .select({ count: count() })
        .from(incidents)
        .where(
          or(
