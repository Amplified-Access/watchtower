...
- **Analytics** (`GET /api/v1/analytics/incidents`, `/analytics/overview`; `handler/analytics.go`, `usecase/analytics`) aggregates the same public anonymous reports the maps draw — counts of reports, injuries and fatalities, optionally grouped by country, type, day, week or month. `groupBy` values map to a fixed table of SQL expressions (`groupExpressions`), so no caller-supplied text reaches the query; that, plus the bucket limit, is what makes it safe to expose to the chat assistant. The filter clauses are shared with the maps through `mapFilterSQL`, so both always count the same rows — its columns are qualified `r.` because the aggregates join `incident_types`, which has `created_at` and `description` columns of its own. Results are cached under the existing `anon:map:version`, so a new report strands stale aggregates too.
...
- **The chat assistant answers data questions through tools, not SQL.** `app/api/chat/route.ts` gives the model four tools over the Go API — `getDataOverview`, `queryIncidentStats` (`lib/api/analytics.ts`), `findIncidents` and `getIncidentDetail` (`lib/api/map.ts`) — alongside the knowledge-base `getInformation`. A text-to-SQL agent was considered and rejected: the same database holds accounts, sessions and organization-internal reports, and the public shapes deliberately omit evidence keys. The tool schemas mirror the Go handler's accepted values, so the model can't ask for a grouping or window the backend would reject. **Each tool call is a separate model request**, so one data question can cost three to nine requests against the Gemini quota (the free tier allows 20 requests per day per model).
