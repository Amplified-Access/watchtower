...
# branch moved since we last fetched. Railway (backend) and Vercel (frontend)
# both deploy on push to `staging`/`production` — see README.md's
# "CI/CD and Branching" section.
...
if [[ "$DEPLOY_BRANCH" != "staging" && "$DEPLOY_BRANCH" != "production" ]]; then
  echo -e "${RED}Usage: $0 staging|production${RESET}" >&2
  exit 1
fi
...
echo ""
echo -e "${YELLOW}About to force-push (with lease) ${DEPLOY_BRANCH} to ${REMOTE}. This will trigger a ${DEPLOY_BRANCH} deploy.${RESET}"
